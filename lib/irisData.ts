...
export const SPECIES_COLORS: Record<IrisSpecies, string> = {
  Setosa: "#818cf8",
  Versicolor: "#fb923c",
  Virginica: "#34d399",
};

export const SPECIES_LIST: IrisSpecies[] = ["Setosa", "Versicolor", "Virginica"];

// [sepalLength, sepalWidth, petalLength, petalWidth] — প্রতি species-এর ৩০টা করে, মোট ৯০টা
// আসল Fisher's Iris Dataset থেকে নেওয়া বাস্তব মাপ (সংক্ষিপ্ত কিন্তু প্রকৃত মান)
const RAW: [number, number, number, number, IrisSpecies][] = [
  // Setosa
...
export const irisDataset: IrisFlower[] = RAW.map(
...
// প্রতিটা feature-এর min/max, slider-এর রেঞ্জ ঠিক করতে ব্যবহার হবে
export const FEATURE_RANGES = {
  sepalLength: { min: 4.0, max: 8.0, step: 0.1 },
  sepalWidth: { min: 1.8, max: 4.5, step: 0.1 },
  petalLength: { min: 1.0, max: 7.0, step: 0.1 },
  petalWidth: { min: 0.1, max: 2.6, step: 0.1 },
};